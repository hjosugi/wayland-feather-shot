...
描いた注釈は後から編集できます。**選択**ツール（`V`）で図形をクリック——
中身が空の四角形の内側や、斜め矢印のバウンディングボックスの空白をクリックしても
その下にある本当の対象が選ばれます——8つのハンドルで**リサイズ**、四隅の外側の
ハンドルで**回転**、Shift+クリックやラバーバンドで複数選択、矢印キーで微調整、
`Ctrl+↑` / `Ctrl+↓`で重ね順を変更。<kbd>Shift</kbd>を押しながらだと角リサイズは
縦横比維持、回転は15°スナップ、新規の四角は正方形、移動は軸ロックになります。

キャンバスにはカメラがあります。**`Ctrl`+ホイール**または
**`Ctrl` + `+` / `-`** でズーム（10 %〜1600 %）、ホイールや中ボタンドラッグで
パン、**`Ctrl+1`**で全体表示、**`Ctrl+0`**で等倍。4Kのキャプチャも縮小表示のまま
勘で描くのではなく、ピクセル単位で注釈できます。

線幅・文字サイズ・番号バッジの大きさは基準画像サイズに対して指定されるため、
同じ設定が1080pでも4Kでも同じ見た目になります。
